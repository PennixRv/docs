...
mem-recall 让 AI 在用户提到过往对话时自动调用 `trellis mem`，从本地 Claude Code、Codex、Devin CLI、Grok、OpenCode、Pi、ZCode 的会话记录里检索内容，并用 session-id + 原话引用作答。
...
| Claude Code、Codex CLI、Devin CLI、Grok、OpenCode、Pi、ZCode            | 历史会话来源            | 至少一个 |
...
| Devin CLI   | `~/.local/share/devin/cli/sessions.db`（`--platform devin`；不是 `trellis init --devin` Desktop） |
